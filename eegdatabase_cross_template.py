...
class Database:
    ...
    def create_covmats(self, ERP, savepath , verbose=False):
        """
        Load the csv files.
        """
        ...
        for filename in listdir(self.path):
            ...
            self.add_covmats(filename, ERP, verbose)

            covmats_Dict = create_covmats_Dict(self)

            np.save(savepath + 'Covmats_Dict4', covmats_Dict)


    def create_rTNT(self, ERP, centroids_List , save_path, verbose = False):
        """
        Load the csv files.
        ...
        for filename in listdir(self.path):
            ...
            self.add_rTNT(filename, ERP, centroids_List, verbose)
    ...
    def add_covmats(self, filename, ERP, verbose):
        """
        Load files for a given session.
        ...
        return subject.add_session_covmats(session_name, ERP) if session_name not in subject.sessions else False

    def add_rTNT(self, filename, ERP, centroids_list,  verbose):
        """
        Load files for a given session.
...
class Subject:
    ...
    def add_session_covmats(self, session_name, ERP):
        """
        Load csv files for one particular session.
        """
        session = Session(session_name, self.name, self.path, self.delta, self.target, self.nontarget,
                          self.chnames, self.bandpass, self.filtre_order)
        session.read_files_covmats(ERP)
        ...
        for i, t in enumerate(markers['Time(s)']):
            if markers['Identifier'][i] in self.target + self.nontarget:
                ...
                if tmp.shape[1] >= Nt:
                    ...
                    all_epochs.append(trial_cov)
                    if markers['Identifier'][i] in self.target:
                        all_labels.append(0)
                    if markers['Identifier'][i] in self.nontarget:
                        all_labels.append(1)

        self.covmats = np.array(all_epochs)
        self.labels = np.array(all_labels)
